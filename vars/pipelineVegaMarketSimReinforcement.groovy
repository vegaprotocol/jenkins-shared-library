...
void call() {
    ...
    pipeline {
        ...
        environment {
            CGO_ENABLED = 0
            GO111MODULE = 'on'
            DOCKER_IMAGE_NAME_LOCAL = 'vega_sim_learning:latest'
        }
        stages {
            stage('CI Config') {
                steps {
            ...
            stage('Tests') {
                parallel {
                    stage('Learning Tests') {
                        steps {
                            sh label: 'Run Reinforcement Learning Test', script: '''
                                scripts/run-docker-learning.sh ${BUILD_NUMBER}
                            '''
                        }
                    }
                }
                post {
                    always {
                        archiveArtifacts artifacts: 'test_logs/**/replay'
                    }
                }
            }
        } // end: stages
