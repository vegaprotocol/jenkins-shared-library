                            ...
                            script {
                                if (params.VEGA_VERSION) {
                                    sh label: 'copy binaries to ansible', script: """#!/bin/bash -e
                                        cp ./bin/vega ./ansible/roles/barenode71/files/bin/
                                        cp ./bin/data-node ./ansible/roles/barenode71/files/bin/
                                        cp ./bin/visor ./ansible/roles/barenode71/files/bin/
                                    """
                                }
                                ...
                                dir('ansible') {
                                    ...
                                    if (params.ACTION == 'create-node' && !params.SKIP_INFRA_PROVISION) {
                                        stage('Provision Infrastructure') {
                                            sh label: "ansible playbooks/playbook-barenode71-common.yaml", script: """#!/bin/bash -e
                                                ansible-playbook \
                ...
                stages {
                    ...
                    stage('Validate delegation') {
                        steps {
                            ...
                            sleep 30
                            script {
                                def url = "https://api-n00.${env.NET_NAME}.vega.community/api/v2/epoch".replaceAll('fairground', 'testnet')
